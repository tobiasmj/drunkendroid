...
public class MockInsertCallDatabaseConnection implements IDatabaseConnection {
	...
	public Connection getConn() {
		...
		try {
			...
			expect(stmt.executeUpdate("Insert into PhoneCall(trip,timeStamp,latitude,longitude,caller,receiver,endTimeStamp) values (1,1,10.0,10.0,004551883250,004551883250,2)")).andReturn(1);
			stmt.close();
			EasyMock.expectLastCall();
			replay(stmt);
